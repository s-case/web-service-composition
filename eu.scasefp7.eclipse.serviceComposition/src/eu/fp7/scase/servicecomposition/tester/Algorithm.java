package eu.fp7.scase.servicecomposition.tester;
...
import eu.fp7.scase.servicecomposition.importer.OwlImporter;
import eu.fp7.scase.servicecomposition.importer.PythonImporter;
...
import eu.fp7.scase.servicecomposition.transformer.Transformer.ReplaceInformation;
import eu.fp7.scase.servicecomposition.transformer.JungXMItoOwlTransform;
...
public class Algorithm {
	...
	public static class WeightReport {
		...
		public void updateWeight() {
			...
			effectiveProbability = selection.getWeight() / Matcher.getMaxWeight(nameSimilarity);
			nameSimilarity = Math.max(nameSimilarity, selection.getTargetService().getOperation().getName()
					.getComparableForm().split("\\s").length);
			worstCaseProbability = selection.getWeight() / Matcher.getMaxWeight(nameSimilarity);
			this.nameSimilarity = Similarity.similarity(selection.getOperationToReplace().getName(), selection
					.getTargetService().getOperation().getName());
		}
		...
		public String toString() {
			...
			ret += "\tWeight     : " + (int) Math.round(weight) + " (" + (int) Math.round(nameSimilarity)
					+ " name weight)\n";
	...
	/**
	 * <h1>costReport</h1>Class for the calculation of the total cost of the workflow.
	 * @author mkoutli
	 *
	 */
	public static class costReport{
		...
		/**
		 * <h1>calculateWorkflowCost</h1> Calculate the total cost of the workflow by adding the per usage/month/year/unlimited costs calculated by the <code>calculateCost</code> and save it in a string to print it.
		 * @param ret
		...
		public String calculateWorkflowCost(String ret){
			...
			if (this.totalCost!=0){
				...
				return ret;
			//System.out.println(this.totalCost + " " + this.totalCurrency+ret);
			}
			return "";
		}
		...
		/**
		 * <h1>calculateCost</h1>Calculate the total cost of a charge type(e.g. per usage) by adding the costs of the services in EUR, USD, GBP.
		 * @param currency
		 * @param service
		 */
		public void calculateCost(String currency, OwlService service){
			...
			case "EUR":
				totalCostEUR = totalCostEUR
						+ ((Operation) service.getContent()).getAccessInfo().getCommercialCostSchema()
								.getCommercialCost();
				break;
			case "USD":
	...
	/**
	 * <h1>trialReport</h1>Class for calculating the total(minimum) trial period of the workflow.
	 * @author mkoutli
	 *
	 */
	...
	/**
	 * <h1>licenseReport</h1>Class for gathering the total licenses of the workflow.
	 * @author mkoutli
	 *
	 */
	 ...
	 * @throws Exception
	 */
	public static ArrayList<Importer.Operation> importServices(final String owlPath)
			throws Exception {
	...
	public static Graph<OwlService, Connector> transformationAlgorithm(String path,
		...
		while (true) {
			...
			ArrayList<Transformer.ReplaceInformation> replaceInformations = transformer.getReplaceInformation(
					operations, 10);
			...
			if (!selection.getOperationToReplace().getRealOperations().isEmpty()) {
				replaceInformations = transformer.getReplaceInformation(selection.getOperationToReplace()
						.getRealOperations(), selection.getTargetService(), 1);
			...
			String chargeType=((Operation) service.getContent()).getAccessInfo().getCommercialCostSchema().getCostPaymentChargeType();
			...
			//trial period
			int durationInUsages=((Operation) service.getContent()).getAccessInfo().getCommercialCostSchema().getTrialSchema().getDurationInUsages();
			int durationInDays=((Operation) service.getContent()).getAccessInfo().getCommercialCostSchema().getTrialSchema().getDurationInDays();
		...
		while (true) {
			...
			String currency = ((Operation) replacedOperation.getContent()).getAccessInfo().getCommercialCostSchema()
					.getCommercialCostCurrency();
			String chargeType=((Operation) replacedOperation.getContent()).getAccessInfo().getCommercialCostSchema().getCostPaymentChargeType();
			switch (chargeType){
			case "per usage":
			...
			//trial period
			int durationInUsages=((Operation) replacedOperation.getContent()).getAccessInfo().getCommercialCostSchema().getTrialSchema().getDurationInUsages();
			int durationInDays=((Operation) replacedOperation.getContent()).getAccessInfo().getCommercialCostSchema().getTrialSchema().getDurationInDays();
			workflowTrialPeriod.setTrialReport(durationInDays, durationInUsages);
			
			//License
			String licenseName=((Operation) replacedOperation.getContent()).getAccessInfo().getLicense().getLicenseName();
			workflowLicense.setLicenseReport(licenseName);
		...
		//GENERATE CODE
//		String code = (new NonLinearCodeGenerator<FunctionCodeNode>()).generateCode(transformer.getGraph(), "tester", false);
...
//		System.out.println("Workflow code was successfully generated in "+filePath);
...
//		generatedFile.close();
	 ...
	 * @throws Exception
	 */
//	public static void codeGeneration(String path, Graph<OwlService, Connector> graph) throws Exception {
//		String codeGenerated = "";
//		try {
//			System.out.print("Generating linear code... ");
//			codeGenerated = (new eu.fp7.scase.servicecomposition.codeGenerator.LinearCodeGenerator()).generateCode(
//					graph, "tester", true);
//		} catch (Exception e) {
//			System.out.print("Failed\nGenerating class code... ");
//			codeGenerated = (new eu.fp7.scase.servicecomposition.codeGenerator.NonLinearCodeGenerator()).generateCode(
//					graph, "tester", true);
//		}
