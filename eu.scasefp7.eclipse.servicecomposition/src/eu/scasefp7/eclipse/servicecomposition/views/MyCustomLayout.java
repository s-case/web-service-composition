...
public class MyCustomLayout extends AbstractLayoutAlgorithm {
	/**
	 * horizontal spacing
	 */
	private double horSpacing = 40 * 4;
	/**
	 * vertical spacing
	 */
	private double verSpacing = 60;

	public MyCustomLayout(int styles) {
		super(styles);
	}
	 ...
	 */
	private void placeChildless(List<GXMoreThanNode> childlessStrand, GXMoreThanNode parentLeft,
			GXMoreThanNode parentRight, int yLoc) {
		int startMark = 0;
	...
	private void shiftTreesRightOfMark(GXMoreThanNode mark, int shift) {
		...
		List<GXMoreThanNode> treeRoots = leftMostChild.getRow().subList(leftMostChild.getRow().indexOf(leftMostChild),
	...
	private void builtTreeFromRoot(GXMoreThanNode currRoot, List<InternalNode> entitiesList,
			List<InternalRelationship> relationshipsList, List<GXMoreThanNode> currRow,
	...
	private InternalNode getFirstEntity(List<InternalNode> entitiesList, List<InternalRelationship> relationshipsList) {
		...
		for (int i = 0; i < entitiesLeft.size(); i++) {
			if (entitiesLeft.get(i).toString().equals("StartNode"))
				return entitiesLeft.get(i);
		}
