...
/**
 * <h1>WSDLCaller</h1> This class is used as an interface with
 * <code>gr.iti.wsdl.wsdlToolkit</code>.
 * 
...
public class WSDLCaller extends OperationCaller {
	...
	public void callOperation(Operation operation) throws Exception {
		...
		if (parsedWS == null) {
			...
			System.out.println("IMPORTING DOMAIN\t" + operation.getDomain().toString() + " ("
		...
		for (Object input : wsOperation.getHasInput().getHasNativeOrComplexObjects()) {
			// note that if the input is of type ComplexObject you have to
			// recursively fill in the values
			if (input instanceof NativeObject) {
				NativeObject no = (NativeObject) input;
				Value val = null;
				for (Argument arg : operation.getInputs())
					if (no.getObjectName().getLocalPart().equalsIgnoreCase(arg.getName().toString())
							&& arg instanceof Value)
						val = (Value) arg;
				if (val == null)
					throw new Exception(
							"Operation " + operation.getName() + " missing input " + no.getObjectName().getLocalPart());
		...
		InvocationResult result = Axis2WebServiceInvoker.invokeWebService(parsedWS.getWsdlURL(),
				new QName(parsedWS.getTargetNamespaceURI(), wsOperation.getOperationName()), wsOperation.getHasInput(),
				wsOperation, parsedWS);
		System.setOut(original);
		...
		for (Argument output : operation.getOutputs()) {
			for (Argument member : output.getSubtypes()) {
				if (member.getSubtypes().isEmpty())
					((Value) member).setValue("<failed assignment>");
			}
		}

		if (result.getResponseHasNativeOrComplexObjects().get(0) instanceof NativeObject
				&& operation.getOutputs().get(0).getSubtypes().size() > 0) {
	...
	/**
	 * <h1>assign</h1> Performs assignment between a given argument and a WSDL
	 * data structure
	 * 
	 * @param data
	 *            : a WSDL data structure
	 * @param argument
	 *            : the given argument
	 * @param getData
	 *            : true to assign to the given argument, false to assign to the
	 *            WSDL data structure
	 * @throws Exception
	 */
	protected void assign(Object data, Argument argument, boolean getData) throws Exception {
		...
		if (data instanceof ComplexObject) {
			...
			for (Object dat : ((ComplexObject) data).getHasNativeObjects()) {
				...
				for (Argument arg : argument.getSubtypes()) {
					if (arg.getName().getContent().toString().equalsIgnoreCase(
							((NativeObject) dat).getObjectName().toString().substring(retIndex + 1))) {
						assign(dat, arg, getData);
					}
				}
			}
			for (Object dat : ((ComplexObject) data).getHasComplexObjects()) {
				int retIndex = ((ComplexObject) dat).getObjectName().toString().indexOf("}");
				for (Argument arg : argument.getSubtypes()) {
					if (arg.getName().getContent().toString().equalsIgnoreCase(
							((ComplexObject) dat).getObjectName().toString().substring(retIndex + 1))) {
						assign(dat, arg, getData);
					}
				}
			}
		...
		if (argument.getSubtypes().size() > 1
				&& ((InvocationResult) data).getResponseHasNativeOrComplexObjects().size() == 1) {
		...
		if (argument.getSubtypes().size() != ((InvocationResult) data).getResponseHasNativeOrComplexObjects().size())
			throw new Exception("Different number of members between class " + argument.getName() + "("
					+ argument.getSubtypes().size() + " members) and invocation result "
					+ ((InvocationResult) data).toString() + " ("
	...
	protected void assignNative(Object data, Argument argument) throws Exception {
		String xmlString = ((NativeObject) ((InvocationResult) data).getResponseHasNativeOrComplexObjects().get(0))
				.getHasValue();
